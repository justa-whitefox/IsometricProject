...
public class Fractal : MonoBehaviour {
    ...
    static readonly int matricseId = Shader.PropertyToID("_Matrices");
    ...
    void OnEnable() {
        ...
        for (int li = 1; li < parts.Length; li++) { //li = level index
            ...
            for (int fpi = 0; fpi < levelParts.Length; fpi += 5) { //fpi = fractal part iterator
                for (int ci = 0; ci < 5; ci++) { //ci = child index
                    levelParts[fpi + ci] = CreatePart(ci);
                } 
            }
        }

        if(propertyBlock == null) { //this expression can be reduced to: propertyBlock ??= new MaterialPropertyBlock();
    ...
    void Update() {
        ...
        for(int li = 1; li < parts.Length; li++) {
            ...
            for(int fpi = 0; fpi < levelParts.Length; fpi++) {
                ...
                levelMatrices[fpi] = float4x4.TRS(part.worldPosition, part.worldRotation, scale * float3.one);

                Bounds bounds = new Bounds(rootPart.worldPosition, 3f * objectScale * Vector2.one);
                for(int i = 0; i < matricesBuffers.Length; i++) {
                    ComputeBuffer buffer = matricesBuffers[i];
                    buffer.SetData(matrices[i]);
                    propertyBlock.SetBuffer(matricseId, buffer);
